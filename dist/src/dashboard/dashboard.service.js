...
export const getDashboardByIdService = async (userId) => {
    try {
        ...
        const pendingClaims = await db
            ...
            .where(and(eq(referralClaimTable.referrer_id, userId), eq(referralClaimTable.status, 'pending')));
        const claimedClaims = await db
            .select({ count: count() })
            .from(referralClaimTable)
            .where(and(eq(referralClaimTable.referrer_id, userId), eq(referralClaimTable.status, 'claimed')));
        const expiredClaims = await db
            .select({ count: count() })
            .from(referralClaimTable)
            .where(and(eq(referralClaimTable.referrer_id, userId), eq(referralClaimTable.status, 'expired')));
